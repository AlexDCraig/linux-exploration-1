...
// using the alphabet we statically set at the beginning of this file,
// convert a character to its corresponding integer
...
// convert a number to its corresponding char
char convertNumToChar(int n1)
{
	return alphabet[n1];	
}

// return the difference between the alphabet's numbers
int diffAlphabetNums(int n1, int n2)
{
	return n1 - n2;
}

// Decryption algorithm: if difference is below 0, add 27
// get the new alphabet number by modding it by 27
...
{
	// must be sure key is long enough
	...
	// Make a string consisting of the decrypted ciphertext
	for (i; i < length+1; i++)
	{
...
int main(int argc, char *argv[])
{
	...
	while (1)
	{
		...
		{	
			...
			// parent process should periodically check on its kids
			// remember WNOHANG means that it waits for any child pid
			do
			{
