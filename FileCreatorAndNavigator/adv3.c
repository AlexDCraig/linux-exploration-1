...
FILE** generateRoomFiles(char* directory, struct Room** holder)
{
	...
	char path[40]; // Path will look like: (directory)/(roomFile.txt), how we will access these files later for reading
	memset(path, '\0', sizeof(path));
	
	snprintf(path, 39, "%s/%s", directory, "room1.txt"); // generate path to this file by concatenating the directory with the room file name
	...
	snprintf(path, 39, "%s/%s", directory, "room2.txt");
	files[1] = fopen(path, "a+");
	holder[1]->fileName = "room2.txt";

	snprintf(path, 39, "%s/%s", directory, "room3.txt");
	files[2] = fopen(path, "a+");
	holder[2]->fileName = "room3.txt";

	snprintf(path, 39, "%s/%s", directory, "room4.txt");
	files[3] = fopen(path, "a+");
	holder[3]->fileName = "room4.txt";

	snprintf(path, 39, "%s/%s", directory, "room5.txt");
	files[4] = fopen(path, "a+");
	holder[4]->fileName = "room5.txt";

	snprintf(path, 39, "%s/%s", directory, "room6.txt");
	files[5] = fopen(path, "a+");
	holder[5]->fileName = "room6.txt";

	snprintf(path, 39, "%s/%s", directory, "room7.txt");
